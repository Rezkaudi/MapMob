...
import { REGION_PAGE_SIZE, withRegionTable } from './with-region-table';

const TARTUS = buildGovernorate({ id: 'a', name: 'طرطوس' });
const HOMS = buildGovernorate({ id: 'b', name: 'حمص' });

describe('withRegionTable', () => {
  function createStore() {
    const Store = signalStore({ providedIn: 'root' }, withRegionTable());
    return new Store();
  }

  it('shows the number of rows the design draws per page', () => {
    expect(createStore().pageSize()).toBe(REGION_PAGE_SIZE);
  });
  ...
  it('leaves an empty search and sort out of the query', () => {
    ...
    expect(store.currentQuery()).toEqual({ pageIndex: 0, pageSize: REGION_PAGE_SIZE });
  });
  ...
  it('steps back a page once the last row of that page is removed', () => {
    const store = createStore();
    store.showPage({ items: [TARTUS], totalCount: REGION_PAGE_SIZE + 1 });
    store.goToPage(1);
    store.toggleSelected('a');
  ...
  it('stays on the page when other rows remain', () => {
    const store = createStore();
    store.showPage({ items: [TARTUS, HOMS], totalCount: REGION_PAGE_SIZE + 2 });
    store.goToPage(1);
